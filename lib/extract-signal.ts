...
const TOOL: Anthropic.Tool = {
  ...
  input_schema: {
    type: "object",
    properties: {
      found: { type: "boolean", description: "true if a relevant divestiture/sale signal is present" },
      company_name: { type: "string", description: "the asset/company being sold or divested" },
      parent_company: { type: "string", description: "divesting corporate parent (carveouts)" },
      sponsor_firm: { type: "string", description: "PE sponsor exiting (private assets)" },
      sector: {
        type: "string",
        enum: [
          "chemicals", "industrials", "agriculture", "specialty_materials",
          "energy_fuels", "pharma_inputs", "consumer_coatings",
          "aerospace_defense", "capital_goods", "automotive",
          "transportation", "basic_materials",
        ],
        description: "best-fit sector for the asset",
      },
      deal_type: { type: "string", enum: ["carveout", "private_asset"] },
      stage: { type: "string", enum: ["in_market", "monitor_for_exit", "on_hold", "pulled"] },
      confidence: { type: "string", enum: ["high", "medium", "low", "needs_review"] },
      key_quote: { type: "string", description: "the exact sentence that triggered the signal" },
      reasoning: { type: "string", description: "one-sentence justification" },
      revenue: { type: "string", description: "annual revenue if explicitly stated, e.g. \"$1.2B\"" },
      ebitda: { type: "string", description: "EBITDA if explicitly stated, e.g. \"$280M\"" },
      margin: { type: "string", description: "EBITDA margin if stated, e.g. \"23%\"" },
      deal_size: { type: "string", description: "reported or expected deal value, e.g. \"$600M\"" },
    },
    required: ["found"],
  },
};
